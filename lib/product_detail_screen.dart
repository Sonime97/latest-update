...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
 ...
 var cart = Provider.of<DataModel>(context).cart;
  ...
  // Check if the item already exists in the list based on the 'name' property
 // bool itemExists = fill.any((cartItem) => cartItem.name == item.name);
 var vr;
 if (!fill.any((map) =>
      map.keys.every((key) => item.containsKey(key) && map[key] == item[key]))) {
  // await dbHelper.insertData(item);
  await dbHelper.insertData(item);
 var rt = await dbHelper.getAllData();
  setState(() {
    cart = rt;
  });
...
print(item);
 ...
 var it = json.decode(widget.productDetail[0]) as Map;
   final details = it;
    return Scaffold(
